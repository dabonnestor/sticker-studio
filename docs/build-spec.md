...
- **Uniform scaling**: text scales like a shape (§5) — corner handles keep the aspect ratio frozen at the gesture start, so scaling up/down never distorts the glyphs. The gesture **folds into `fontSize` at its end** — the size field in the toolbar is the text's source of truth, so a corner drag lands as a new size, not a lingering scale transform (the wrap width scales with it, the transform resets) — and the box **re-hugs its content at the folded size** while it still auto-fits: a corner scale is a size change, like the toolbar size field, so a scale-down never leaves the box short of the text (auto-fit survives it). A text carrying a folded scale out of a scaled group or multi-selection bakes the same way at ungroup / on selection. The top/bottom handles are hidden (a Y-only drag would distort the glyphs, and the ratio lock pins it dead anyway); the ml/mr handles stay for wrap width (§6). The first manual resize — a wrap-width drag — hands the width to the user: auto-fit stops. (#11, amended 2026-08-17 — the scale folds into `fontSize` at the gesture end and the box re-hugs its content; a corner scale no longer hands the width over)
