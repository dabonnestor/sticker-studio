 ...
 * Uploads opens the Uploads panel — an upload button and the recent-uploads
 * gallery. Choosing an image places it centered, fit to the Document,
 * selected (§1).
 */
export function Sidebar() {
  ...
  const [view, setView] = useState<"default" | "artwork" | "shapes" | "images" | "uploads">(
    "default",
  )

  // Ticket #41 (Variant C): choosing Artwork, Shapes, Images, or Uploads
  // swaps the whole sidebar for that panel. All five stay mounted — the
  // inactive ones are display:none — so a panel's view state (the Graphics
  // and Images panels' search terms, results, and scroll; the Uploads
  // panel's gallery) survives navigating away and back. Only a browser
  // refresh, which remounts the entire app, resets it.
  return (
    <>
      <aside
